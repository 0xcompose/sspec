...
{
    bytes4 public constant maatOracleGlobalPPSInterfaceId =
        bytes4(keccak256("MAAT.V1.Oracle"));

    ///@dev DELTA_PPS_PER_SECOND determines how much the PPS can change over time
    uint256 public immutable DELTA_PPS_PER_SECOND; //RECOMMENDED VALUE: 3 (3 -> 0.25% Per Day -> 94% APR)
    ...
    /* ========== CONSTRUCTOR ========== */
    constructor(address admin, uint256 delta, address addressProvider)
        Ownable(admin)
    ...
    function updateGlobalPPS(
        ...
        for (uint256 i = 0; i < vaults.length; i++) {
            uint112 pricePerShare = globalPricePerShare[vaults[i]].pricePerShare;
            uint32 _lastUpdateTime =
                globalPricePerShare[vaults[i]].lastUpdateTime;

            require(
                _checkDeltaPPS(
                    newPricePerShare[i], pricePerShare, _lastUpdateTime
                ),
    ...
    function getPrevGlobalPPS(address vault)
        external
        view
        returns (uint256 pps, uint32 prevUpdateTime)
    {
    ...
    function getGlobalPPS(address vault)
        external
        view
        returns (uint256 pps, uint32 lastUpdateTime)
    {
    ...
    function _subAbs(uint256 x, uint256 y) private pure returns (uint256) {
        if (x > y) return x - y;
        else return y - x;
    }
}
