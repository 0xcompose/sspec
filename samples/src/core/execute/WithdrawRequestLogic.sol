...
{
    uint256 internal _withdrawCancellationDelay = 1 hours;
    ...
    mapping(bytes32 intentionId => WithdrawRequestInfo) private
        _withdrawRequests;
    ...
    error EmergencyWithdrawalTooMuchWithdrawn(
        uint256 amountWithdrawn, uint256 amountToWithdraw
    );

    /* ======== EXTERNAL ======== */

    function setWithdrawCancellationDelay(uint256 timer) external onlyOwner {
        emit WithdrawCancellationDelayChanged(_withdrawCancellationDelay, timer);

        _withdrawCancellationDelay = timer;
    }

    function setEmergencyWithdrawalDelay(uint256 timer) external onlyOwner {
    ...
    function _createWithdrawRequest(
        ...
        emit WithdrawalRequested(
            shares, estimatedAmountOut, _owner, intentionId, dstEid
        );
    }

    function _cancelWithdrawRequest(bytes32 intentionId)
        internal
        returns (address owner, uint256 shares)
    {
        ...
        require(
            request.creationTime + _withdrawCancellationDelay <= block.timestamp,
            "WithdrawRequestLogic: Not enough time has passed yet to withdraw"
        );
        address _owner = request.owner;

        require(
            msg.sender == _owner, "WithdrawRequestLogic: Unauthorized caller"
        );
    ...
    function getWithdrawRequest(bytes32 intentionId)
        public
        view
    ...
    function withdrawCancellationDelay() public view returns (uint256) {
        return _withdrawCancellationDelay;
    }

    function _validateWithdrawRequestExistence(bytes32 intentionId)
        internal
        view
    {
        WithdrawRequestInfo memory request = _withdrawRequests[intentionId];
