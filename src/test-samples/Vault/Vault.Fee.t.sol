...
contract MaatVaultFeeTesting is MaatVaultTestSetup {
    ...
    function testFuzzing_FeeDeposit(uint256 amountIn, uint64 feeIn) public {
        ...
        uint256 predictedShares =
            (maatVault.convertToShares(amountIn) * (10 ** 8 - feeIn)) / 10 ** 8;
    ...
    function testFuzzing_FeeMint(uint256 shares, uint64 feeIn) public {
        vm.assume(feeIn > 10 ** 6 && feeIn < 5 * 10 ** 6);
        ...
        vm.assertApproxEqAbs(
            maatVault.balanceOf(feeTo), shares - predictedShares, 10
        );
        vm.assertEq(
            token.balanceOf(address(maatVault)),
    ...
    function testFuzzing_FeeWithdraw(uint256 assets, uint64 feeOut) public {
        ...
        vm.assertEq(
            ...
            maatVault.calculateFee(
                maatVault.convertToSharesByLowerPPS(assets), feeOut
            )
        );

        vm.assertApproxEqAbs(
            token.balanceOf(buyer), initialTokenBalanceBuyer + assets, 10
        );
    }

    function testFuzzing_FeeRedeem(uint256 shares, uint64 feeOut) public {
        vm.assume(feeOut > 10 ** 6 && feeOut < 5 * 10 ** 6);
        vm.assume(shares > 10 ** 6 && shares < 10 ** 30);
        ...
        vm.assertEq(
            maatVault.balanceOf(buyer), initialSharesBalanceBuyer - shares
        );
        vm.assertEq(
            maatVault.balanceOf(feeTo), maatVault.calculateFee(shares, feeOut)
        );
        vm.assertApproxEqAbs(
    ...
    function testFuzzing_FeeRequestWithdraw(uint256 shares, uint64 feeOut)
        public
    {
        uint256 initialTokenBalanceBuyer = token.balanceOf(buyer);
        uint256 initialSharesBalanceBuyer = maatVault.balanceOf(buyer);
    ...
    function testFuzzing_FeePreviewDeposit(uint256 assets, uint64 feeIn)
        public
    {
        vm.assume(feeIn > 10 ** 6 && feeIn < 5 * 10 ** 6);
        ...
        uint256 predictedShares = maatVault.convertToShares(assets)
            - maatVault.calculateFee(maatVault.convertToShares(assets), feeIn);

        assertEq(shares, predictedShares);
    }

    function testFuzzing_FeePreviewMint(uint256 shares, uint64 feeIn) public {
        vm.assume(feeIn > 10 ** 6 && feeIn < 5 * 10 ** 6);
        ...
        uint256 predictedAssets = maatVault.convertToAssets(
    ...
    function testFuzzing_FeePreviewWithdraw(uint256 assets, uint64 feeOut)
        public
    {
        vm.assume(feeOut > 10 ** 6 && feeOut < 5 * 10 ** 6);
        vm.assume(assets > 10 ** 4 && assets < 10 ** 30);
    ...
    function testFuzzing_FeePreviewRedeem(uint256 shares, uint64 feeOut)
        public
    {
        vm.assume(feeOut > 10 ** 6 && feeOut < 5 * 10 ** 6);
        vm.assume(shares > 10 ** 4 && shares < 10 ** 30);
        ...
        uint256 predictedAssets = maatVault.convertToAssetsByLowerPPS(
