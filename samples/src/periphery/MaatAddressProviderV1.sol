...
contract MaatAddressProviderV1 is
    ...
    ERC165Registry
{
    bytes4 public addressProviderInterfaceId;
    bytes4 public maatVaultInterfaceId;
    bytes4 public strategyInterfaceId;
    bytes4 public maatOracleGlobalPPSInterfaceId;
    ...
    function oracle() external view override returns (address) {
        require(
            _oracle != address(0), "MaatAddressProviderV1: Oracle is not set"
        );
        return _oracle;
    }
    ...
    function changeStargateAdapter(address newStargateAdapter)
        external
        override
        onlyAdmin
    {
    ...
    function changeSharesBridge(address newSharesBridge)
        external
        override
        onlyAdmin
    {
    ...
    function _removeAddress(address[] storage array, address addressToRemove)
        internal
    {
    ...
    event StrategyRegistered(
        address strategy, bytes32 strategyId, IStrategy.StrategyParams params
    );
    ...
    event IncentiveControllerChanged(
        address prevIncentiveController, address newIncentiveController
    );
    event StargateAdapterChanged(
        address prevStargateAdapter, address newStargateAdapter
    );
    event SharesBridgeChanged(
        address prevSharesBridge, address newSharesBridge
    );
