...
contract VaultEmergencyWithdrawalTest is MaatVaultTestSetup {
    ...
    uint256 initialBalanceShares = 975902173542;
    ...
    function _afterSetUp() internal override {
        ...
        maatVault.depositInStrategy(strategyId, maatVault.idle(), bytes32("intentionId"));
        vm.stopPrank();
    }

    function testFuzz_EmergencyWithdraw_SingleChain(uint256 sharesWithdrawRequest) public {
        ...
        (IExecutor.ActionInput[] memory withdrawInputs, uint256 amountOutTokens) =
            _makeActionInput(1, sharesWithdrawRequest);

        _emergencyWithdraw_template(
            sharesWithdrawRequest, maatVault.chainEid(), owner, receiver, withdrawInputs, amountOutTokens, false
        );
    }

    function testFuzz_EmergencyWithdraw_CrossChain(uint256 sharesWithdrawRequest) public {
        vm.assume(sharesWithdrawRequest <= initialBalanceShares);
        vm.assume(sharesWithdrawRequest > maatVault.minAmount());
        ...
        _emergencyWithdraw_template(
            sharesWithdrawRequest, secondMaatVault.chainEid(), owner, receiver, withdrawInputs, amountOutTokens, false
        );
    }

    function testFuzz_EmergencyWithdraw_SeveralActions(uint256 sharesWithdrawRequest) public {
        vm.assume(sharesWithdrawRequest <= initialBalanceShares);
        vm.assume(sharesWithdrawRequest > maatVault.minAmount());
    ...
    function test_EmergencyWithdraw_TooMuchWithdrawn() public {
        ...
        _emergencyWithdraw_template(
            amountToWithdraw, maatVault.chainEid(), owner, receiver, withdrawInputs, amountOutTokens, true
        );
    }
    ...
    function test_EmergencyWithdraw_EarlyWithdrawal() public {
        ...
        bytes32 intentionId = maatVault.requestWithdraw(initialBalanceShares / 2, maatVault.chainEid(), owner, receiver);

        vm.expectRevert();
        maatVault.emergencyWithdraw(intentionId, new IExecutor.ActionInput[](0));
    }

    function _makeActionInput(uint256 _withdrawAmount, uint256 _sharesWithdrawRequest)
        internal
        view
        returns (IExecutor.ActionInput[] memory withdrawInputs, uint256 amountOutTokens)
    {
        withdrawInputs = new IExecutor.ActionInput[](_withdrawAmount);

        amountOutTokens = maatVault.convertToAssetsByLowerPPS(_sharesWithdrawRequest);
    ...
    function _emergencyWithdraw_template(
        ...
        bytes32 intentionId = maatVault.requestWithdraw(_sharesWithdrawRequest, _chainEidToWithdraw, _owner, _receiver);
        ...
        assertEq(token.balanceOf(_receiver), _initialBalanceReceiver + _amountOutTokens);
        assertEq(maatVault.balanceOf(_owner), _sharesInitialBalanceOwner - _sharesWithdrawRequest);
