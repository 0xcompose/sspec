...
contract SharesBridgeSetup is LayerZeroHelper {
    ...
    function setUp() public override {
        ...
        bridgeA = new MaatSharesBridge(
            address(addressProviderA), address(endPointA), address(this)
        );

        bridgeB = new MaatSharesBridge(
            address(addressProviderB), address(endPointB), address(this)
        );
    ...
    function _deployERC20(string memory _name, string memory _symbol)
        internal
        returns (ERC20Mock token)
    {
        token = new ERC20Mock(_name, _symbol);
    }

    function _mintVaultShares(MaatVaultV1 vault, address to, uint256 amount)
        internal
    {
