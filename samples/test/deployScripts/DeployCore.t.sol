...
import {LiFiExecutorProxy} from "../../src/periphery/LiFiExecutorProxy.sol";
import {MaatVaultV1} from "../../src/core/MaatVaultV1.sol";
import {MaatAddressProviderV1} from
...
contract DeployCoreTests is TestUtils, DeployCore {
        ...
        returns (MaatAddressProviderV1 provider)
    {
        ...
        provider =
            _deploy_MaatAddressProviderV1(create3Factory, admin, admin, forSalt);
    ...
    function _deployOracle(address addressProvider)
        public
        returns (MaatOracleGlobalPPS oracle)
    {
        string memory forSalt = "MAAT.1.MaatOracleGlobalPPS";

        oracle = _deploy_MaatOracleGlobalPPS(
            create3Factory, admin, 116, addressProvider, forSalt
        );
    }

    function test_deployLiFiExecutorProxy() public {
        fork(42161, 239650040);

        address addressProvider = address(_deployProvider());
        address executor = address(0x123);

        LiFiExecutorProxy proxy = _deploy_LiFiExecutorProxy(
            create3Factory,
            addressProvider,
            executor,
            address(this),
        ...
        assertEq(
            proxy.executor(), executor, "LiFiExecutorProxy: Executor mismatch"
        );
    }
}
