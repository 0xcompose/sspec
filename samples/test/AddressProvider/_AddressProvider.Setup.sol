...
contract MaatAddressProviderTestSetup is TestUtils {
    ...
    address stargateAdapter = address(uint160(uint256(keccak256("stargateAdapter"))));
    ...
    function setUp() public virtual {
        ...
        TransparentUpgradeableProxy maatAddressProviderProxy =
            new TransparentUpgradeableProxy(address(_implementation), admin, "");
        ...
        maatAddressProvider = MaatAddressProviderV1(address(maatAddressProviderProxy));
        maatAddressProvider.initialize(admin);

        oracle = new MaatOracleGlobalPPS(admin, 10 ** 20, address(maatAddressProvider));

        bytes4 interfaceId = bytes4(keccak256("MAAT.V1.AddressProvider"));

        require(maatAddressProvider.supportsInterface(interfaceId), "AddressProvider: something went wrong");
    ...
    function _setUpVault(ERC20 token, address _addressProvider) internal returns (MaatVaultV1) {
        return new MaatVaultV1(address(this), address(token), 0, _addressProvider, commander, watcher, 1);
    }

    function _setUpYearnV3Strategy(ERC20 token) internal returns (YearnV3Strategy) {
        IStrategyFromStrategies.StrategyParams memory strategyParams =
            IStrategyFromStrategies.StrategyParams(42161, "Yearn", 3, address(token), address(0));

        return new YearnV3Strategy(strategyParams, address(0), feeToPerformance, performanceFee);
    }
