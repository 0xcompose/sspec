...
abstract contract FeeManager is Ownable {
    ...
    uint256 public constant maxFee = 5 * 10 ** 6;
    ...
    event FeeChanged(
        uint64 prevFeeIn, uint64 newFeeIn, uint64 prevFeeOut, uint64 newFeeOut
    );
    ...
    function _calculateFee(uint256 amount, uint112 fee)
        internal
        pure
        returns (uint256)
    {
