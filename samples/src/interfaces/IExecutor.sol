...
interface IWithdrawRequestLogic {
    ...
    event WithdrawCancellationDelayChanged(uint256 prevDelay, uint256 newDelay);
    event EmergencyWithdrawalDelayChanged(uint256 prevDelay, uint256 newDelay);
...
interface IBridgeLogic {
    ...
    event BridgeFinished(uint256 amount, uint32 originEid, bytes32 intentionId);

    /* ====== FUNCTIONS ====== */

    function finishBridge(
        uint256 amountBridged,
        uint256 bridgeAmountBeforeFees,
        uint32 originEid,
        bytes32 intentionId
    ) external;
}
...
interface IStrategyManager {
    ...
    function getStrategyById(bytes32 _strategyId)
        external
        view
        returns (address, bool);

    function getStrategyByAddress(address _strategy)
        external
        view
...
interface IExecutor is IWithdrawRequestLogic, IBridgeLogic, IStrategyManager {
    ...
    event DepositedInStrategy(
        bytes32 strategyId, uint256 amount, bytes32 indexed intentionId
    );
